...
class CUB2011(Dataset):
    ...
    url = 'https://data.caltech.edu/records/65de6-vp158/files/CUB_200_2011.tgz' # 'http://www.vision.caltech.edu/visipedia-data/CUB-200-2011/CUB_200_2011.tgz'
    file_id = '1hbzc_P1FuxMkcabkgn9ZKinBwW683j45'
    filename = 'CUB_200_2011.tgz'
    tgz_md5 = '97eceeb196236b17998738112f37df78'

    def __init__(self, root, train=True, transform=None, download=True, loader=default_loader, base_set=None):
        self.root_og = root
...
def cub200_train_loader():
    return CUB2011(root='/scr/cub200', train=True, transform=cub200_train_transforms)


def cub200_test_loader():
    return CUB2011(root='/scr/cub200', train=False, transform=cub200_test_transforms)


def prepare_train_loaders(config):
    return {
        'full': torch.utils.data.DataLoader(
            CUB2011(root='/scr/cub200', train=True, transform=cub200_train_transforms),
...
def prepare_test_loaders(config):
    loaders = {
        'full': torch.utils.data.DataLoader(
            CUB2011(root='/scr/cub200', train=False, transform=cub200_test_transforms),
            batch_size=config['batch_size'],
            shuffle=False,
            num_workers=config['num_workers']
        )
