...
def stanford_dogs_train_loader():
    return StanfordDogs('/scr/stanford_dogs/', train=True, transform=stanford_dogs_train_transform)


def stanford_dogs_test_loader():
    return StanfordDogs('/scr/stanford_dogs/', train=False, transform=stanford_dogs_test_transform)


def prepare_train_loaders(config):
    return {
        'full': torch.utils.data.DataLoader(
            StanfordDogs('/scr/stanford_dogs/', train=True, transform=stanford_dogs_train_transform),
...
def prepare_test_loaders(config):
    loaders = {
        'full': torch.utils.data.DataLoader(
            StanfordDogs('/scr/stanford_dogs/', train=False, transform=stanford_dogs_test_transform),
            batch_size=config['batch_size'],
            shuffle=False,
            num_workers=config['num_workers']
        )
