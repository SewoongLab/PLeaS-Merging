...
class NABird(Dataset):
    ...
    def __init__(self, root, train=True, transform=None, unpack=True, loader=default_loader, base_set=None):
        self.root_og = root
    ...
    def unpack(self):
        ...
        if not os.path.exists(os.path.join(self.root, self.base_folder)):
            print('Extracting...')
            fake_root = '/'.join(self.root_og.split("/")[:-1])
            os.system(f'tar -xzf "/gscratch/sewoong/anasery/datasets/nabirds/nabirds.tar.gz" -C "{fake_root}"')
            print('Done')
    
    def _load_metadata(self):
        images = pd.read_csv(os.path.join(self.root, 'images.txt'), sep=' ',
                             names=['img_id', 'filepath'])
        image_class_labels = pd.read_csv(os.path.join(f'{self.root}/image_class_labels.txt'),
...
def nabird_train_loader():
    return NABird(root='/scr/nabirds', train=True, transform=nabird_train_transforms)


def nabird_test_loader():
    return NABird(root='/scr/nabirds', train=False, transform=nabird_test_transforms)


def prepare_train_loaders(config):
    return {
        'full': torch.utils.data.DataLoader(
            NABird(root='/scr/nabirds', train=True, transform=nabird_train_transforms), 
...
def prepare_test_loaders(config):
    loaders = {
        'full': torch.utils.data.DataLoader(
            NABird(root='/scr/nabirds', train=False, transform=nabird_test_transforms), 
            batch_size=config['batch_size'], 
            shuffle=config['shuffle_train'], 
            num_workers=config['num_workers']
        )
