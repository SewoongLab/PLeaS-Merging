...
def _split_cifar(train_ds, label_split: int):
    ...
    s1 = {
        "images_u8": np.concatenate(
            (lt_images_u8[:5000], gte_images_u8[5000:]), axis=0
        ),
        "labels": np.concatenate((lt_labels[:5000], gte_labels[5000:]), axis=0),
    }
    s2 = {
        "images_u8": np.concatenate(
            (gte_images_u8[:5000], lt_images_u8[5000:]), axis=0
        ),
...
class ImageNet:
    def __init__(self, shuffle=True, batch_size=128):
        ...
        if [ ! -d "/tmp/train" ]; then
            set -e
            cd /tmp
            tar xf /gscratch/sewoong/jhayase/data/imagenet-val.tar -C /tmp/
            tar xf /gscratch/sewoong/jhayase/data/imagenet-train.tar.gz -C /tmp/
        fi
        """
        ...
        # Data loading code
        kwargs = {"num_workers": min(4, len(os.sched_getaffinity(0)))}
